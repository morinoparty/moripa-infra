...
Ubuntu 25.10 以降の sudo は sudo-rs でプロンプトの形が違い Ansible の become が
そのままでは通らないため、`base` ロールの先頭で同じパスワードを stdin から渡して
NOPASSWD の sudoers を先に書く(2 回目以降は何もしない)。
...
|---|---|---|---|
| site1-node1 / node2 / node3 | control-plane ×3 | 10.100.0.11 / .12 / .13 | 10.200.1.11 / .12 / .13(VIP .10) |
...
<Callout title="netplan apply は wg 越しに実行される" type="warn">
  ...
  (`ManageForeignRoutingPolicyRules=no`)を先に入れて networkd を再起動し、
  apply 直後にハブへの ping で経路が生きていることを検査する(node-bootstrap も
  同じ drop-in を入れる)。拠点を絞るには `make cluster ANSIBLE_ARGS='--limit site1'`。
...
flowchart TD
    ...
    B --> B2["longhorn_prereq: ルート LV を VG 全体に拡張 / iscsid / nfs-common(全ノード)"]
    B2 --> C["site1 control-plane 構築(node1: kubeadm init → node2, node3 が join、serial 1)"]
    C --> D["site2 control-plane 構築(node1: kubeadm init)"]
    D --> E["worker join(site2 の node2)"]
...
site1 は 3台とも control-plane で、node1 の `kubeadm init` のあと node2・node3 が
`--control-plane` で順に join する(`serial: 1` で etcd メンバー追加の同時実行を
避ける)。site2 は node1 の 1台で、node2 が worker として join する。Cilium の投入は
両拠点の先頭 control-plane に対して最後にまとめて行う。Longhorn 本体は Ansible では
入れず、ArgoCD bootstrap 後に `longhorn` Application が入れる(`/docs/admin/argocd`)。
...
- ✅ `ip rule` に priority 200 の除外ルール(cluster_lan / Pod / Service CIDR)が3本(Cilium が local テーブルを pref 100 へ動かすため、それより後ろ)
...
- ✅ Caddy: 規約に沿った名前(例 `test.site1.dev.morino.party`)で
  `curl -v https://<host>` が対象拠点の Envoy に届く(Ingress 未作成なら
...
- ✅ Caddy 経由のリクエストで、アプリ側に `X-Forwarded-For` としてクライアントの
  実IPが先頭に入って届く(末尾はハブの `10.100.0.1`)
- ✅ Pod 内から大きな HTTPS レスポンスを取得できる(MSS clamp / MTU の検証)
- ✅ ノード同士で LAN 越しに `curl -m 5 http://10.200.1.<N>/` すると timeout する
  (`ingress-filter`: wg0 / lo 以外からの `:80` を drop)。ハブからの
  `curl -H "Host: <host>" http://10.100.0.1<N>/` は届く
...
## 旧「/data LV」からの統合手順(参考)

2026-09-12 までは VG の残りを `data` LV にして `/data` にマウントしていた。現在はルート LV を
VG 全体に広げ、`/data/longhorn` はルート FS 上のディレクトリ。`longhorn_prereq` ロールは
`/dev/ubuntu-vg/data` が残っていると止まるので、その場合はノードごとに次を行う
(3 レプリカなので 1 台ずつなら無停止):

```sh
# 1) Longhorn: そのノードのスケジュールを止め、ノード上のレプリカを消す(他 2 台に残る)
kubectl -n longhorn-system patch nodes.longhorn.io site1-nodeN --type=merge -p '{"spec":{"allowScheduling":false}}'
kubectl -n longhorn-system get replicas.longhorn.io -o json \
  | jq -r '.items[] | select(.spec.nodeID=="site1-nodeN") | .metadata.name' \
  | xargs -r kubectl -n longhorn-system delete replicas.longhorn.io
# 2) ノードで LV を統合(longhorn-disk.cfg は必ず残す。消すと別ディスク扱いになる)
sudo cp -a /data/longhorn /root/longhorn-keep && sudo umount /data
sudo lvremove -y ubuntu-vg/data && sudo lvextend -l +100%FREE ubuntu-vg/ubuntu-lv && sudo resize2fs /dev/ubuntu-vg/ubuntu-lv
sudo sed -i '\|/dev/ubuntu-vg/data /data|d' /etc/fstab
sudo rm -rf /data && sudo mkdir /data && sudo mv /root/longhorn-keep /data/longhorn
# 3) Longhorn: スケジュールを戻し、OS 用に 30GiB を予約。レプリカは自動で再構築される
kubectl -n longhorn-system patch nodes.longhorn.io site1-nodeN --type=merge \
  -p '{"spec":{"allowScheduling":true,"disks":{"<disk名>":{"path":"/data/longhorn","allowScheduling":true,"storageReserved":32212254720}}}}'
kubectl -n longhorn-system get volumes.longhorn.io   # robustness が healthy に戻ってから次のノードへ
```
