...
`reverse_proxy` ロール(Caddy。`proxy_routes` から Caddyfile を生成)、
...
- `nft list ruleset` で masquerade / MSS clamp / `iifname "wg0" oifname "wg0" accept`
  と、input chain の `tcp dport { 80, 443 } accept`・`tcp dport 25565 accept` が入っている
- `systemctl status caddy` が active。`proxy_routes` が空のうちは Caddy は
  どのポートも listen しない(`curl http://<linode_ip>` は接続拒否になる)
...
- ✅ HAProxy: 外部から `nc -vz <linode_ip> 25565` が通る(Minecraft デプロイ後。
  `allowed_sources` を設定済みなら Velocity のホストから)。
  `ssh moripa@10.100.0.1 'echo "show stat" | sudo socat stdio /run/haproxy/admin.sock'`
  で backend の両ノードが UP
- ✅ Caddy: `proxy_routes` に最初のホストを登録して DNS を向けたあと、
