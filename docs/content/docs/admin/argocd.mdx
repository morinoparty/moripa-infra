...
site1 の ArgoCD は `https://argocd.site1.dev.morino.party/` で開ける(HTTPRoute は
`kubernetes/sites/site1/bootstrap/argocd/httproute.yaml`、TLS は Linode の Caddy が終端し
...
変更したら `kubectl apply -k kubernetes/sites/site1/bootstrap/argocd` を再実行する。
...
HTTPRoute を multi-source で同期する。ノード側の前提(`/data` LV、iscsid、nfs-common)は
