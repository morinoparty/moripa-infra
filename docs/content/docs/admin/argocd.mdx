...
| 1 | `monitoring` | kube-prometheus-stack(Prometheus / node-exporter / Grafana) |
...
Ingress を multi-source で同期する。ノード側の前提(ルート LV の拡張、`/data/longhorn`、iscsid、nfs-common)は
...
## Ingress の health について

Cilium の ingress controller は hostNetwork(:80)+ ClusterIP で動かしているため、Ingress の
`status.loadBalancer` は空のまま。ArgoCD 既定の health check はこれを Progressing と判定し、
sync が「waiting for healthy state of Ingress」で止まるので、`kubernetes/common/argocd/argocd-cm-patch.yaml`
で Ingress を常に Healthy とみなす health override を入れている。Ingress の実際の疎通は
ハブから `curl -H "Host: <host>" http://10.100.0.1<N>/` で確認する。
