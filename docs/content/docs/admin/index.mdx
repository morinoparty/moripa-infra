...
flowchart TD
    ...
    D --> E["Ansible: cluster 構成(wg spoke → k8s → Cilium)"]
...
flowchart LR
    ...
    subgraph remote["管理者の担当(すべてリモート)"]
        K["wg 本設定"] --> L["kubeadm init/join"] --> M["Cilium 投入"] --> N["ArgoCD bootstrap"]
    end
    onsite --> remote
```
