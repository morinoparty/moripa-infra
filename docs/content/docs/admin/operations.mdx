...
落ちると Ansible からも SSH からも到達できなくなる。復旧手段:

1. 同一 LAN 内の別ノード経由で入る(`ssh moripa@10.100.0.12` → LAN IP へ)
2. それも不可なら現地でコンソールログインし、node-bootstrap スクリプトを
   再実行(`make node-bootstrap HOST=...` で再生成して渡す)

また、ノードは DHCP のため**ルーターの MAC 予約を消さないこと**。予約が
外れて LAN IP が変わると kubeadm/etcd が壊れる(クラスタ再構築級)。
ルーター交換時は先に予約を移すこと。
