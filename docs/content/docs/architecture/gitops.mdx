...
└── sites/
    ...
    │   │   ├── ingress/            # Cilium Gateway API(hostNetwork :80、TLS は Linode の Caddy)
    │   │   ├── storage/            # Longhorn の values + UI の HTTPRoute
    │   │   └── monitoring/
...
| 0 | `ingress` | Cilium Gateway API(hostNetwork、:80。Caddy からの転送を受ける) |
| 0 | `longhorn` | 分散ブロックストレージ(既定 StorageClass)。chart は charts.longhorn.io、values と HTTPRoute は git(multi-source)。`prune: false` |
| 1 | `headlamp` | k8s の Web UI(`headlamp.site1.private.dev.morino.party`、oauth2-proxy の後ろでトークンログイン) |
| 1 | `monitoring` | 監視 |
